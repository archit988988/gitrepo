...
public class nfs2 extends JComponent implements ActionListener, MouseMotionListener, MouseListener,KeyListener{
	  BufferedImage im,hand1,hand2,hand,ball,bat,divider,mycar,whichtree,car,playagain,tree2,car2,whichcar;
	   public  nfs2() throws IOException
...
//	g.fillRect(300, 200, wi, hi);
	g.drawImage(playagain, 300, 200, wi, hi, null);
     ...
     AudioClip song=JApplet.newAudioClip(getClass().getResource(""));
    
     AudioClip song1=JApplet.newAudioClip(getClass().getResource(""));
     
     AudioClip song2=JApplet.newAudioClip(getClass().getResource(""));
     AudioClip song3 = song1;
	private int t=5;
	double gs;
	private int mycarz=20;
	private int zcon=1;
	...
	for(int i=0;i<4;i++)
	{
	  if(mycarx+mycarw>ax1[i]+40&&mycary<ay1[i]+bh[i]-40&&zcon!=0)
	  {  
		  //song3=song;
		 mycarw=0;
		 mycarh=0;
	  ...
	  if(mycarx+mycarw>carx&&mycarx<carx+carw&&mycary<cary+carh&&mycary+mycarh>cary&&zcon!=0)
	  {  //song3=song;
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		if(zcon==0)
		{
		mycarw=mycarw+mycarz;
		mycarh=mycarh+mycarz;
		mycarz=mycarz-4;
		if(mycarz<=-20)
		{
			zcon=1;
