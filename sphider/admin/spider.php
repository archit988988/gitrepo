...
********************************************/
	...
	if ($all ==  1) {
		...
		if ($reindex == 1 && $command_line == 1) {
			$result=mysqli_query($success,"select url, spider_depth, required, disallowed, can_leave_domain from ".$mysql_table_prefix."sites where url='$url'");
			echo mysql_error();
	...
	function index_url($url, $level, $site_id, $md5sum, $domain, $indexdate, $sessid, $can_leave_domain, $reindex) {
		...
		if (strstr($url_status['state'], "Relocation")) {
			...
			if ($url <> '') {
				$result = mysqli_query($success,"select link from ".$mysql_table_prefix."temp where link='$url' && id = '$sessid'");
				echo mysql_error();
		...
		/*		if ($newdomain <> $domain)
				...
				if ($OKtoIndex == 1) {
					...
					if (isset($domain_arr[$domain_for_db])) {
						...
						mysqli_query($success,"insert into ".$mysql_table_prefix."domains (domain) values ('$domain_for_db')");
					...
					if (is_array($wordarray) && count($wordarray) > $min_words_per_page) {
						if ($md5sum == '') {
							...
							echo mysql_error();
							$result = mysqli_query($success,"select link_id from ".$mysql_table_prefix."links where url='$url'");
							echo mysql_error();
	...
	function index_site($url, $reindex, $maxlevel, $soption, $url_inc, $url_not_inc, $can_leave_domain) {
		...
		if (!isset($all_keywords)) {
			$result = mysqli_query($success,"select keyword_ID, keyword from ".$mysql_table_prefix."keywords");
			echo mysql_error();
		...
		if ($site_id != "" && $reindex == 1) {
			...
			echo mysql_error();
			$result = mysqli_query($success,"select url, level from ".$mysql_table_prefix."links where site_id = $site_id");
			while ($row = mysql_fetch_array($result)) {
				$site_link = $row['url'];
			...
			mysql_query ($success,"insert into ".$mysql_table_prefix."sites (url, indexdate, spider_depth, required, disallowed, can_leave_domain) " .
		...
		$result = mysqli_query($success,"select site_id, temp_id, level, count, num from ".$mysql_table_prefix."pending where site_id='$site_id'");
		echo mysql_error();
		$row = mysql_fetch_row($result);
		$pending = $row[0];
		$level = 0;
		...
		if ($pending == '') {
			...
			mysqli_query($success,"select temp_id, level, count from ".$mysql_table_prefix."pending where site_id='$site_id'");
			echo mysql_error();
			$sessid = $row[1];
			$level = $row[2];
			$pend_count = $row[3] + 1;
		...
		while (($level <= $maxlevel && $soption == 'level') || ($soption == 'full')) {
			...
			$result = mysqli_query($success,"select distinct link from ".$mysql_table_prefix."temp where level=$level && id='$sessid' order by link");
			echo mysql_error();
			...
			while ($count < count($links)) {
				...
				if ($forbidden == 0) {
					...
					if ($rows == 0) {
						...
						mysqli_query($success,"update ".$mysql_table_prefix."pending set level = $level, count=$count, num=$num where site_id=$site_id");
						echo mysql_error();
	...
	function index_all() {
		...
		$result=mysqli_query($success,"select url, spider_depth, required, disallowed, can_leave_domain from ".$mysql_table_prefix."sites");
		echo mysql_error();
	...
	function get_temp_urls ($sessid) {
		global $mysql_table_prefix;
		$result = mysqli_query($success,"select link from ".$mysql_table_prefix."temp where id='$sessid'");
		echo mysql_error();
		$tmp_urls = Array();
    	while ($row=mysql_fetch_row($result)) {
			$tmp_urls[$row[0]] = 1;
	...
	function get_domains () {
		global $mysql_table_prefix;
		$result = mysqli_query($success,"select domain_id, domain from ".$mysql_table_prefix."domains");
		echo mysql_error();
		$domains = Array();
    	while ($row=mysql_fetch_row($result)) {
			$domains[$row[1]] = $row[0];
